...
function BulkImport(items, updateExisting, disableAutomaticIdGeneration) {
    ...
    function transformDocument(doc) {
		if (doc["actorTags"]) {
			doc["actorTags"] = doc["actorTags"].replace('[','').replace(']','').replace(/"/g,'').split(",");
		}
		if (doc["movieTags"]) {
			doc["movieTags"] = doc["movieTags"].replace('[','').replace(']','').replace(/"/g,'').split(",");
		}
		if (doc["genreTags"]) {
			doc["genreTags"] = doc["genreTags"].replace('[','').replace(']','').replace(/"/g,'').split(",");
		}
        return doc;
    }
